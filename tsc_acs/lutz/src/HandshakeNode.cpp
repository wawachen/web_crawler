...
HandshakeNode::HandshakeNode() : qos(0), sendRate(20), qosTimeout(0, 60*1000*1000),
							  frameCount(0), seed(0), podSeed(0), sessionId(0)
{
	ReportVersion(node);
	sub = node.subscribe("pod_handshake", 10, &HandshakeNode::rxCallback, this);

	pubACS_HANDSHAKE = node.advertise< ::lutz::Handshake>("acs_handshake", 10, true);
}

void HandshakeNode::rxCallback(const ::lutz::Handshake & msg)
{
	msgHandshakeIn = msg;
